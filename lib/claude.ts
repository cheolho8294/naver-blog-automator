...
이후 블로그는 **사진·메모 기반 작업 후기**로 쓰이며, 현장 디테일러가 고객에게 설명하듯 담백하게 작성된다.
...
- topic: 시공/작업 한 줄 주제.
- keywords: 쉼표로 구분된 검색 키워드 다섯 개 내외. 후기·방법·비교 등 검색 의도를 대표하게.
- notes: 1인칭. 입고→전처리→메인→마무리 흐름. **관찰**(어디에 무엇이 보였는지)·**왜 그렇게 했는지**·현장 체감 1~2문장. 「일반적으로/보통」 위주 설명 금지. 사진·메모 우선. **확인되지 않은** 구체 공정·약제(점토바, 철분제 등)는 확정형으로 쓰지 말고 포괄 표현. 메모·사진에 있는 것만 구체적으로.
- comparison: 시공 전 vs 후 또는 기존 vs 이번, 한두 문장. 현실적 표현(눈에 띄게 개선 등). 사진·메모 근거.
- 작업자 메모(workerMemo)는 자동 채우기 대상이 아님. JSON에 workerMemo 키를 넣지 말 것. 입력돼 있으면 notes 작성 시 차종·특이사항·약제 참고만.
- 작업 시간·RPM·희석비·코팅 지속 등 수치는 메모에 있을 때만.
- JSON 외 텍스트 금지.`;
...
작업자 메모(참고만, 자동 채우기·수정 대상 아님): ${prefilled.workerMemo || "(비어있음)"}
...
작업자 메모 (차종·특이사항·사용약제 등 — 작성자 직접 입력, 사실 근거): ${form.workerMemo || "(없음)"}
...
[작업 후기 · 현장 서술]
- 사진·사용자 메모·**작업자 메모**(차종·특이사항·사용약제) 중심. 작업자 메모에 적힌 차종·약제·특이사항은 사실 근거로 반영하고, 메모에 없는 항목은 사진만으로 추측하지 말 것. 흔한 오염·스월·미세 스크래치는 자연스럽게 가능. 깊은 손상·PPF·코팅박리 등 → 사진·메모 있을 때만.
- **확인되지 않은 세부 작업·약제 확정 금지**: 메모·사진에 없는 구체 공정·약제·도구(점토바, 철분제 등)는 확정형으로 쓰지 말고 포괄 표현(필요한 전처리, 도장면 상태에 맞는 전처리 등). 메모·사진에 명시된 것만 구체적으로.
- **전문용어**: 존재하지 않거나 현장에서 쓰지 않는 용어 생성 금지. 현장·업계 통용 표현만.
- 「일반적으로/보통/주로/대부분」 백과사전식 최소 → **이번 차량 관찰·현장 경험** 우선. 공정마다 **왜 했는지** 포함.
- 사진으로 확인 가능한 **차량 특징**(넓은 측면, 대형 전면, 슬라이딩 도어 등) 본문 반영. **현장 체감 1~2문장** 필수(구체적 관찰).
- FAQ: 해당 작업(광택·코팅·실내·에바·가죽 등) **고객 상담형** 질문 우선.
- 결과·흐름·수치·말투: 기존 규칙(현실적 표현, 입고→최종, 수치는 근거 있을 때, 현장에서 말할 수 있는 문장만).
